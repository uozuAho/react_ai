...
import { City, Colour, all_colours } from 'src/games/pandemic/pandemic_board';
import { IterUtils } from 'src/libs/array/iter_utils';
import { PandemicGameState, CityState } from 'src/games/pandemic/game_state';

interface IPandemicStateDisplayProps {
    game_state: PandemicGameState;
}

export class PandemicStateDisplay extends React.Component<IPandemicStateDisplayProps> {
    ...
    private fit_board_to_svg(svg: SVG.Doc) {
        ...
        const xcoords = Array.from(IterUtils.map(this.props.game_state.get_cities(), c => c.city.location.x));
        const ycoords = Array.from(IterUtils.map(this.props.game_state.get_cities(), c => c.city.location.y));
    ...
    private drawBoard() {
        const graph = this.props.game_state.get_city_graph();
